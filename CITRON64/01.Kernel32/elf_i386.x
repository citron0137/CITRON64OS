...
SECTIONS
{
  ...
  /* 여기부터 앞쪽으로 이동된 코드 영역 */
  /* 시작주소(0x10200) 지정, .text가 속한 라인 최 상단으로 이동, 패딩값(?)(0x90909090) 지정 */
  .text 0x10200   :
  {
  ...
  /* 여기부터 앞쪽으로 이동된 Read Only 영역 */
  .rodata         : { *(.rodata .rodata.* .gnu.linkonce.r.*) }
  .rodata1        : { *(.rodata1) }
  /* 여기까지 앞쪽으로 이동된 Read Only 영역 */


  /* 데이터 영역의 시작을 섹터 단위로 맞춤 */
  . = ALIGN (512);

  /* 여기부터 앞쪽으로 이동된 데이터 영역 */
  .data           :
  {
  ...
  .data1          : { *(.data1) }
  /* 여기까지 앞쪽으로 이동된 데이터 영역 */

  /* 여기서부터 앞쪽으로 이동된 bss 영역 */
  __bss_start = .;
  .bss            :
  {
   *(.dynbss)
...
.plt.sec        : { *(.plt.sec) }
  ...
  /* 여기부터 책에 없어서 당황스러운 부분 */
  . = .;
  . = DATA_SEGMENT_END (.);
  /* 여기까지 책에 없어서 당황스러운 부분 */
